...
from utils import enum_to_onehot as make_onehot
from evaluations import score

def vis_img(img, maxh=512, maxw=512, debuglevel=0, bgr=True):
    if debuglevel > 2:
        print("vis_img")

    # debug
    vis = img*.4 #/2 vis[0] is handwriting for some reason here. Rendered as blue (plt) or red (cv2).
    #vis[:,:,:2] = 0
    #vis[:,:,:1] = 0 # Invisible!!!!!
    #return vis[:,:,:3]
