...
from keras.layers import Dropout, Activation, LeakyReLU, Layer, SeparableConv2D
...
def build_simple_hourglass(num_classes=6, ds=4, init_feats=32, feature_growth_rate=4, ks=[(5,5),(3,3),(3,3),(3,3),(3,3)], loss='mse', lr=0.002, dropout_rate=0.5, use_transpose_conv=False, input_channels=3, model_save_path="model_checkpoint.h5", conv=SeparableConv2D):
    x = inputs = Input(shape = (None, None, input_channels))
    x = Conv2D(init_feats, ks[0], activation='linear', padding='same', use_bias=False, name='conv1_1')(x)
    x = keras.layers.LeakyReLU(alpha=0.00)(x)
    nf = init_feats
    for layer in range(ds):
        x = conv(nf, ks[1+layer], padding='same', strides=(1,1), use_bias=False)(x)
        x = keras.layers.LeakyReLU(alpha=0.00)(x)
        x = BatchNormalization()(x)
        x = MaxPooling2D(pool_size=(2,2), strides=(2,2))(x) #AveragePooling2D
        x = Dropout(dropout_rate)(x)
        nf += feature_growth_rate
    x = conv(nf, ks[1+layer], padding='same', use_bias=False)(x)
    x = keras.layers.LeakyReLU(alpha=0.00)(x)
    x = BatchNormalization()(x)
    for layer in range(ds):
        x = UpSampling2D(size=(2, 2))(x)
        x = conv(nf, (3,3), padding='same', use_bias=False)(x)
        x = keras.layers.LeakyReLU(alpha=0.00)(x)
        nf -= feature_growth_rate
