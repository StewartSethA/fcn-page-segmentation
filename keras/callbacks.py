...
import time
from postprocessing import postprocess_preds, multihot_to_multiindexed_rgb
from visuals import show
    ...
    def on_batch_end(self, batch, logs={}):
        ...
        if self.batch_num % self.interval == 0:
            ...
            if self.training_generator.gt is not None:
                print "   GT"
                plt.figure(3)
                v = visuals.vis_img(self.training_generator.gt, bgr=True)
                plt.imshow(v)
                #cv2.imshow('GT', v)
                #cv2.waitKey(10)
            if self.training_generator.image is not None:
                ...
                if self.model is not None:
                    ...
                    plt.imshow(visuals.vis_img(self.pred, bgr=True))
            plt.pause(0.001)
...
class DisplayAccuracyCallback(Callback):
    def __init__(self, generator=None, generator_class=None, training_generator_class=None, pixel_counts_by_class=defaultdict(lambda:0), eval_interval=50):
...
class LogTimingCallback(Callback):
    def __init__(self, batch_size=6, **kwargs):
