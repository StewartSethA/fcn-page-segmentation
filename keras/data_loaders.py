...
from gt_loader import load_gt, index_training_set_by_class
...
class PerformanceFeedbackDatasetSampler(DatasetSampler):
    ...
    def get_image_path_and_class(self, c=None, debuglevel=0):
        ...
        randind = 0 if len(self.class_to_samples[c]) == 1 else np.random.randint(0, len(self.class_to_samples[c]))
...
class ImageAndGTBatcher(object):
    def __init__(self, training_folder, num_classes=4, batch_size=1, downsampling_rate=1, dataset_sampler=None, image_sampler=None, data_augmenter=DataAugmenter(), image_channels=3, train=True, max_downsampling=8, cache_images=True, crop_size=256, index=None):
    ...
    def generate(self, subcrop_dim=None, batch_size=None, compress_in_ram=False, cache_in_ram=True, debuglevel=0):
        ...
        while True:
            ...
            for b in range(batch_size):
                ...
                #if self.samples_obtained % 100 == 0:
                #if self.samples_obtained % 100 == 0:
                self.image = image
                self.gt = gt
            ...
            if debuglevel > 2:
                print("Format batch time:", format_batch_time)
