...
for leaveout in A B C; do
    ...
    valdir=$root_data_dir/$author/$leaveout
    pwd
    echo $data_dir
    mkdir -p $data_dir
    mkdir -p $valdir
    for leavein in A B C; do
        for subauthor in Barrett Iain Zhihan; do
            if [ "$leaveout" != "$leavein" ]; then
                for f in `ls $root_data_dir/$subauthor/$leavein/`; do
                    cp $root_data_dir/$subauthor/$leavein/$f $data_dir/"${subauthor}_${f}"
                done
            else
                for f in `ls $root_data_dir/$subauthor/$leavein/`; do
                    cp $root_data_dir/$subauthor/$leavein/$f $valdir/"${subauthor}_${f}"
                done
            fi
        done
    done
